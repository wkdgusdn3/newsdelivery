...
def sendDailyDelivery():
    ...
    query = "SELECT email, title, company, url FROM delivery_log, user, crawling_news WHERE user.seq = delivery_log.user_seq AND crawling_news.seq = delivery_log.news_seq AND crawling_news.date BETWEEN '%s' AND '%s' ORDER BY email" %('2016.09.19', '2016.09.20')
    cur.execute(query)
